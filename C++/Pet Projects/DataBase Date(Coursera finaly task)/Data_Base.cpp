...
{
    return ostr << setfill('0') << setw(4) << date.GetYear() << "-" << setw(2) << date.GetMonth() << "-" << setw(2)
                << date.GetDay();
}
int ConvertDateToDays(const Date& date) { return date.GetDay() + date.GetMonth() * 31 + date.GetYear() * 365; }
...
bool operator<(const Date& lhs, const Date& rhs) { return ConvertDateToDays(lhs) < ConvertDateToDays(rhs); }
