...
double get_token()
{
    ...
    case '9':
    case '.':
        number_value = atof(&buff[buffI - 1]);
    ...
    default:
        if (isalpha(ch))
        {
            char* p = name_string;
            *p++ = ch;
            while (buff[buffI++] && isalnum(ch))
                *p++ = ch; // cin.get()
            buffI--;       // cin.putback(ch);
            *p = 0;
...
name* look(const char* p, int ins)
{
    ...
    while (*pp)
        hash_code = hash_code << 1 ^ *pp++; // all symbols
