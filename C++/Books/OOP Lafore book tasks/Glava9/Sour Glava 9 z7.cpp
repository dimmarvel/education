...
{
public:
    CountPostf() : Counter() // конструктор без параметров
    {
    }
    CountPostf(int c) : Counter(c) // конструктор с одним параметром
    {
    }
    CountPostf operator++(int) { return CountPostf(count++); }
};
///////////////////////////////////////////////////////////
int main()
{
    ...
    c3++;
    c3++;
    cout << "\nc3 = " << c3.get_count(); // показываем результат

    --c2;
    --c2;                              // уменьшаем c2
    cout << "c2 = " << c2.get_count(); // показываем результат

    CountDn c4 = --c2;                   // создаем переменную c3 на основе c2
    cout << "\nc3 = " << c4.get_count(); // показываем значение
