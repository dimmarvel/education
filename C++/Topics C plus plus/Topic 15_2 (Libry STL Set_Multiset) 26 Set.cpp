// set.cpp
// Множество, хранящее объекты типа string#pragma warning (disable:4786) //для работы с множествами //(только для компиляторов Microsoft)
#include <iostream>
#include <set>
#include <string>
using namespace std;
...
{                           // Массив строковых объектов
	...
	nameSet.insert("Robert");   // никакого эффекта: такой элемент уже имеется
	nameSet.insert("Barry");
	nameSet.erase("Mary");      // удаление элемента вывод размера множества
	cout << "\nРазмер=" << nameSet.size() << endl;
	iter = nameSet.begin();     // вывод элементов множества
	while (iter != nameSet.end())
		cout << *iter++ << '\n';

	string searchName;          //получение искомого имени от пользователя
	cout << "\nВведите искомое имя: ";
	cin >> searchName;
