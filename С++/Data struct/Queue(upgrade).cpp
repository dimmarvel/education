...
using namespace std;
enum state_queue {WORRY = -1, CHILL = 2};
...
void worry(vector<state_queue> &queue, int operation)
{
	queue[operation] = WORRY;
}

void quite(vector<state_queue> &queue, int operation)
{
	queue[operation] = CHILL;
}
