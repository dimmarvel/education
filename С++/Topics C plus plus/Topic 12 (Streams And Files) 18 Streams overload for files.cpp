...
private:
	...
	float inches;
public:
	Distance() : feet(0), inches(0.0) {} // конструктор (без аргументов) конструктор (два аргумента)
...
{
	char dummy;   // дл€ ('), (-) и (") с помощью перегруженного >>
...
int main()
{
	...
	while (true)
	{
		...
		if (ifile.eof())     // выход по EOF
			break;
		cout << "–ассто€ние = " << dist1 << endl; //вывод рассто€ний
	}
	system("system");
	return 0;
}