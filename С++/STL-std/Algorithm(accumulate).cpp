...
int main() {
	...
	auto it3 = accumulate(begin(v), end(v), 1, [](int a, int b) {
		return a * b;
	});
	auto it4 = accumulate(begin(v), end(v), 1, [](int a, int b) {
		return (b % 2 == 0) ? a * b : a;
	});
	string it5= accumulate(next(begin(v)), end(v),to_string(v[0]), [](string a, int b) {
